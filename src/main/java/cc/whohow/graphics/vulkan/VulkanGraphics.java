package cc.whohow.graphics.vulkan;
...
import static cc.whohow.graphics.vulkan.VKUtil.*;
...
//    }
    ...
    private static long createPipeline(VkDevice device, long renderPass, VkPipelineVertexInputStateCreateInfo vi) throws IOException {
        ...
        shaderStages.get(0).set(loadShader(device, "cc/whohow/graphics/vulkan/triangle.vert.spv", VK_SHADER_STAGE_VERTEX_BIT));
        shaderStages.get(1).set(loadShader(device, "cc/whohow/graphics/vulkan/triangle.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT));
...
//        glfwWindowHint(GLFW_DECORATED, GLFW_FALSE);
        ...
        GLFWWindowSizeCallback windowSizeCallback = new GLFWWindowSizeCallback() {
            public void invoke(long window, int width, int height) {
                if (width <= 0 || height <= 0)
                    return;
                VulkanGraphics.width = width;
                VulkanGraphics.height = height;
